...
main() {
  ...
  create_mr_status="$(auth_curl "${OWNER_TOKEN}" -o "${TMP_DIR}/mr.json" -w '%{http_code}' \
    -X POST "${MR_BASE}" \
    -H 'Content-Type: application/json' \
    -d '{"title":"E2E merge request","body":"closes #1","sourceRef":"feature/mr-e2e","targetRef":"main","isDraft":false}')"
  ...
  step "Verifying auth matrix (public read, outsider write denied)"
  ...
  step "Checking changes endpoint and linked discussions endpoint availability"
