...
for (const a of cj.artboards) {
  ...
  a.h = h + 24
  await ctx.close()
  console.log(a.file.padEnd(20), h)
}
await browser.close()
// keep directions on one row baseline, bottom-aligned under the cover
const y = 900
for (const a of cj.artboards) if (a.file !== 'Main.dc.html') a.y = y
